						...
						withDockerContainer(args: '--entrypoint=\'\'', image: 'docker/compose:1.23.2', toolName: 'Default') {
							withEnv(["IMAGE_SUFFIX=${imagesuffix}"]) {
								switch(env.BRANCH_NAME) {
								  case "master":
								    sh 'NETWORK_NAME=production && chmod 777 ./docker/scripts/configure-network.sh && sh ./docker/scripts/configure-network.sh'
									...
									break
								  case "staging":
								  	sh 'NETWORK_NAME=staging && chmod 777 ./docker/scripts/configure-network.sh && sh ./docker/scripts/configure-network.sh'
									...
									break
								  case "testing":
								  	sh 'NETWORK_NAME=testing && chmod 777 ./docker/scripts/configure-network.sh && sh ./docker/scripts/configure-network.sh'
									...
									break
								  case "develop":
								  	sh 'NETWORK_NAME=development && chmod 777 ./docker/scripts/configure-network.sh && sh ./docker/scripts/configure-network.sh'
